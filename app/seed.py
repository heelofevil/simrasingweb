...
FAQ_ITEMS: list[tuple[str, str, int]] = [
    (
        "Сколько стоит сборка под ключ?",
        "Итог зависит от выбранного железа, города и формата установки. В конфигураторе видна ориентировочная сумма — точный расчёт инженер пришлёт после заявки.",
        1,
    ),
    (
        "Можно ли собрать сетап самому по частям?",
        "Да. В конфигураторе можно выбрать готовую сборку или собрать комплект по категориям — руль, база, педали, кокпит и остальное.",
        2,
    ),
    (
        "Как работает доставка и установка?",
        "В своём городе инженер приезжает, собирает кокпит и настраивает FFB на месте. В других регионах — доставка и удалённая настройка по видеосвязи.",
        3,
    ),
    (
        "Сколько времени занимает сборка?",
        "Срок зависит от состава и наличия железа. После заявки инженер подтвердит ориентир по срокам и дате установки.",
        4,
    ),
    (
        "Нужно ли платить онлайн на сайте?",
        "Нет. Сначала оставляешь контакт и получаешь расчёт — оплата и детали согласуются с инженером напрямую.",
        5,
    ),
]
...
def seed_faq() -> dict:
    ...
    db.session.commit()
    return {"seeded": True, "count": len(FAQ_ITEMS)}
